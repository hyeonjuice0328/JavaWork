...
public class File11Main {
	public static void main(String[] args) {
		...
		try (
			...
			fw.write(charArr); // UTF-8 인코딩의 경우 한글은 한글자당 3바이트 영문은 1바이트로 저장.
//			★ 문자기반 출력시만 꼭 끝에 flush() 해주자 ★
			fw.flush(); // write() 호출 후에는flush()하여 출력 버퍼의 데이터가 완전히 출력되도록 한다. 오류가 나지 않도록!! 
			
			// 읽기 
			char [] buff = new char[100];
			//읽어들일 문자의 갯수를 담을 변수 charsRead
			int charsRead = 0; //읽어들인 문자 갯수
			charsRead = fr.read(buff); // 몇글자 읽어들였는지 리턴 
			for (int i = 0; i < charsRead; i++) {
				System.out.println(buff[i]);
			}
			System.out.println();
			System.out.println("읽은 문자 개수 : " + charsRead); // 글자 단위로 읽기 
