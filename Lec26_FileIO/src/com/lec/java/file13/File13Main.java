...
//			out = new PrintWriter(bw); // printwriter 장착!!
			
			out = new PrintWriter(new BufferedWriter(new FileWriter(FILE1)));
			test_write(out);
			//읽는 것은 printreader 라는 것은 없다 bufferedreader로 읽으면 된다. 
			System.out.println();
			br = new BufferedReader(new FileReader(FILE1)); //bufferedreader로 부터 읽어들인다. 
			test_read(br);
			//---------------------------------------------------------------------------
			out.close();
			br.close();
			...
			System.out.println("현재 OS인코딩" + System.getProperty("file.encoding")); //>>결과 현재 OS인코딩UTF-8
			//euc-kr로 인코딩 저장 																	OutputStreamWriter의 두번째 매개변수: "euc-kr" 
			out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream(FILE2),"euc-kr" )));
			test_write(out);
			System.out.println();
			                                                                  //인코딩 된 방식으로 읽어 들여야 읽어진다. !!!! 
	...
	//bufferedreader로 한라인씩 읽어들이기 
	public static void test_read(BufferedReader br) {
		//읽어들여서 String buffer 객체에 추가 
		String line;
		StringBuffer sb = new StringBuffer();
		try {
			while((line= br.readLine()) != null) { // 줄의 개념은 \n 줄바꿈 문자 앞에서 끊는다. 다 읽으면 null을 리턴 
				sb.append(line + "\n");
			}
		} catch (IOException e) {
			e.printStackTrace();
