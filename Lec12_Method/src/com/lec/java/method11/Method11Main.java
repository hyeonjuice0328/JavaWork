...
//		double [] arr = new double[n]; //new 는 heap 메모리에 생긴다. 
									   //Heap 메모리 용량 초과OutOfMemoryError
	...
	// 재귀 호출은 무한정 불가 ! : StackOverflowError stack의 메모리 용량 초과 "예외가 아니라 에러!!!!" 
	public static void showNumber(int n) {
		System.out.println(n); // 일단 화면에 출력
		showNumber(n + 1); // 받은 값에 1을 더해서 자기자신 다시 호출
	}
