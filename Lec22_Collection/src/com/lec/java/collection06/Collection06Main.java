...
public class Collection06Main {
	...
	public static void main(String[] args) {
		...
		// 데이터 검색:
		// Set은 인덱스가 없기 때문에 get() 메소드를 제공하지 않습니다. 순서대로 정리되있는 개념이 없어서 .. 
		...
		//   element 없다면 false 리턴  // 2번째 삭제가 아닌 데이터 2를 삭제 
		System.out.println("삭제결과: " + set.remove(2));
		System.out.println("삭제결과: " + set.remove(2));
		
		System.out.println();
		System.out.println("삭제 후:");
		itr = set.iterator(); //iterator은 일회용이다 사용후 다시 사용하려면 이렇게 재선언해주어야한다. 
		while(itr.hasNext()) {
			System.out.println(itr.next());
		}
		...
		// 그냥 지우고 다시 쓰고 마음대로 편하게 하면 되는 개념 
		// 삭제(remove) 후 추가(add)하면 변경하는 효과
		//500 > 555 로 변경하고 싶은 경우 
		set.remove(400);
		set.add(555);
