...
public class Crawl06Main {
	...
	public static final String FILEPATH = "download"; // 파일들 다운받아 놓을 파일의 경로를 상수로 설정
	
	public static void main(String[] args) throws IOException {
		System.out.println("웹 리소스 다운로드");
		
		Crawl06Main app = new Crawl06Main(); // 인스턴스 생성
...
//		System.out.println();
//		for (int i = 0; i < imgUrls.length; i++) app.download1(movUrls[i]); 
...
//		for (int i = 0; i < dataUrls.length; i++) app.download2(dataUrls[i]); 
	...
	// 방법1
		...
		public void download1(String srcUrl) throws IOException {
			...
			url = new URL(srcUrl);
			//url로부터 stram 뽑아내는 방법
			in = url.openStream();  // URL 로 부터 InputStream 을 뽑아내는 작업!! [핵심]
			
			dstFile = fileNameFromURL(url);
			
			out = new FileOutputStream(FILEPATH + File.separator + dstFile); // 저장 파일 경로 
		...
		// 장점: 파일사이즈, 타입, 이름 등을 미리 알수 있다. >> 사이즈에 맞게 버퍼 만들어 놓을 수 있음 > 속도 향상 될 수 있다. 
		public void download2(String srcUrl) throws IOException {
			...
			String contentType = conn.getContentType(); //파일종료 MIME-TYPE 확인 가능
		
			// 다운로드 받는 파일 이름 확인 가능
			String raw = conn.getHeaderField("content-Disposition"); // url 내부에 정보가 있는 것이 아니라 자료의 헤더에 위치해있다. 
			...
			in = conn.getInputStream(); // HttpURLCOnnection 에서 InputStream을 뽑는다.
			bin = new BufferedInputStream(in);
			out = new FileOutputStream(FILEPATH + File.separator + dstFile);
			bout = new BufferedOutputStream(out);
			
			byte [] buf = new byte[contentLength]; //다운로드 받을 파일 크기 만큼의 버퍼 준비!
			int byteImg;
