...
public class Thread02Main {
	...
	public static void main(String[] args) {
		...
		// 3. Runnable을 구현하는 클래스의 인스턴스를 생성
		Runnable runnable1 = new MyRunnable("안녕 자바프레임워크");
		...
		}).start(); //runnable 익명클래스 .start();
		
		// Runnable 인터페이스는 run() 가상메소드 하나만 가지고 있는 인터페이스 이기 때문에
		// Lambda-expression 으로도  구현 가능.추상메소드 하나 만 있는 경우에만 사용가능
		//System.out.println();
		//System.out.println("Lambda-expression 으로 Runnable 구현");
		new Thread(()->{for (int i = 0; i < 100; i++) {
			//Thread.currentThread() : 현재 이 코드를 실행중인 Thread 객체 리턴
			System.out.println(Thread.currentThread().getName() + i + " : " + "쉬는시간");
		}
...
class MyRunnable implements Runnable {
	...
	public void run() {
		for (int i = 0; i < 100; i++) {
			//Thread.currentThread() : 현재 이 코드를 실행중인 Thread 객체 리턴
			System.out.println(Thread.currentThread().getName() + count + " : " + msg);
			//getName() Thread 안에 있떤 것들은 Thread를 상속받지 못해서 사용불가
			//Thread.currentThread().getName() 이렇게 사용가능
			count++;
		}
