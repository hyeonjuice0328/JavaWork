...
/*  StringBuffer, StringBuilder
	 ...
	 String 클래스는 immutable(-사전적의미:불변의) 클래스이고,-주소값이 아니랑 String 객체 안에 있는 값이 별할 수 없다는 의미 
	 자바에서 기본으로 제공하는 문자열 (StringBuffer, StringBuilder)는 mutable(-사전적의미:변화기쉬운) 클래스임


	 StringBuffer:
		Java ver 1.0부터 있던 클래스
		쓰레드에 안전(thread-safe) 예시-다니는 도로에 신호등이 있다, 속도가 느림
	
	 StringBuilder:
		Java 5부터 생긴 클래스
		쓰레드에 안전하지 않다(thread-unsafe) 예시-다니는 도로에 신호등이 없다, 속도가 빠름.
...
public class String03Main {
	...
	public static void main(String[] args) {
		...
		StringBuffer buff1  = new StringBuffer("Hello"); // literal 로 만들 수 없고 new 를 사용해서 만들어야 한다. 
		StringBuffer buff2 = buff1;
		
		System.out.println("buff1 : " + buff1);
		System.out.println("buff2 : " + buff2);
		
		if(buff1 == buff2) { // 주소값 비교 (레퍼런스타입)
			System.out.println("== 동일참조");
		} else {
			System.out.println("== 다른참조");
		}
