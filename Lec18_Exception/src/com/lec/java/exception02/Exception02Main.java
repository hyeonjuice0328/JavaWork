...
public class Exception02Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("반복문으로 예외(Exception) 처리");
		...
		System.out.println("try~catch 로 만들어 처리");
		
		try { // 수행블럭 (예외발생하면 수행하지않고 catch부로 넘어간다)
			result = num1 / num2;
			System.out.println("결과 : " + result);
		} catch (ArithmeticException ex) { //try블럭에서 발견한 exception이 ArithmeticException 이면 예외처리를 해준다.  
			//예외처리 블럭 : 예외 발생 시 수행
			System.out.println("0으로 나누는 Exception 발생");
			System.out.println(ex.getMessage()); // exception message 
			ex.printStackTrace(); // 수행할 결과 
			//예외가 처리죄면(handling) 프로그램 종료되지 않고 계속 진행이 된다. 
		}
