...
--인덱스가 없으면 테이블에서 특정값을 찾기 위해 모든 데이터를 다 뒤지는 TABLE SCAN 발생 : 자주 발생하게 되면 굉장히 비효율적 
--기본키를 위한 인덱스를 기본 인덱스,   
...
-- 프라이머리 키가 설정되어있으면 프라이머리키가 인덱스의 역할을 했다. 



-- 사원의 인덱스 
-- 직원키의 primary key
-- ROWID : 오라클 객체 / 데이터가 인서트 될때마다 고유한 rowid 값이 생성된다. 
SELECT empno, ROWID FROM t_emp;
SELECT empno, ROWID FROM t_emp ORDER BY EMPNO DESC;
SELECT ename, ROWID
FROM T_EMP ; -- ROWID 순서대로 출력됨

-- 인덱스 <key, rowid> 에 입력
-----------------------------------------------------------
-- (1) B-tree 인덱스 : 기본적으로 사용하고 있는 인덱스 
--	① UNIQUE INDEX
--  ② Non Unique INDEX
...
-- UNIQUE INDEX 설정되면 중복된 값 입력 안된다. 
INSERT INTO t_dept2 values(9100, '임시매장', 1006, '서울지사');
INSERT INTO t_dept2 values(9101, '임시매장', 1006, '부산지사');
--에러: ORA-00001: unique constraint (SCOTT0316.IDX_DEPT2_DNAME) violated
